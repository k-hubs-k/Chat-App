...
const Profile = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8081/profil/" + id)
      .then((res) => {
        setuser(res.data[0]);
  ...
  const backG = () => {
    if (user.images != null) {
      return {
        backgroundImage:
          'url("http://localhost:8081/images/' + user.images + '")',
      };
    } else {
      return {
        backgroundColor: "#FFFFFF",
        backgroundImage:
          "linear-gradient(180deg, #FFFFFF 0%, #f0f0f0 50%, #e2e2e2 100%)",
      };
    }
  };
    ...
    <>
      <div className="wrapper">
        <div className="profil-top">
          <Avatar
            alt={user.username}
            src={"http://localhost:8081/images/" + user.images}
